...
def apply_postfit_shape(histogram, up, down, shift, shift_err):
    ...
    '''
    print histogram, shift, shift_err
...
if __name__ == "__main__":
    ...
    for bin in args.bins:
        ...
        for name in histogram_names:
		    ...
		    print p_shape_unc[name][shape]
                    apply_postfit_shape(histo, up, down, shape_shift, p_shape_unc[name][shape])
		    #apply_postfit_shape(histo, up, down, shape_shift, 0)#!!!!!!!!!!!!!!

            # Apply normalization scale
	    #print p_weight
	    testname=name.replace("125","")
	    #print testname, name
            if testname in p_weight:
                log.info("Scaling %s/%s by %0.3f +- %0.3f",
                         bin, name, p_weight[testname], p_unc[testname])
                apply_postfit_normalization(histo, p_weight[testname], p_unc[testname])
